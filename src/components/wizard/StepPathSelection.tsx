 ...
 * Horizontally swipeable cards, one per path. Tap "Choose This Path" to confirm.
 * Channeling path triggers a follow-up spell selection.
 */
...
interface Props {
  ...
  baseUnitId: string
  equipment: string[]
  baseStats?: Record<string, number> // user-entered stats for this unit
...
// Brief thematic description for each path — shown on the card
...
// Signature stat label per path (roll 7 "OR improve X")
...
// ─── Stat ceiling helpers ────────────────────────────────────────────────────

// Stats that are relative to the base unit's value
...
// Stats that heroes start with / that have absolute ceilings
...
// Hero starting values for MWF (they all start at 1)
...
// Display label and ordering for the stat ceiling grid
...
function computeCeilings(
  ...
  return CEILING_STATS.map(({ key, label, isTargetNumber }) => {
    ...
    if (
      ...
      if (key in rel) {
        if (baseStats) {
          // We have actual stats — show the concrete ceiling
          const base = baseStats[key] ?? 0
          ceiling = base + rel[key]
          isHighlighted = rel[key] > 1
        } else {
          // No stats yet — show relative bonus as "+N"
    ...
    // Shoot is a target number (lower = better), display as e.g. "3+"
                  ...
                  return (
                    <Box sx={{ mb: 1.5 }}>
                      ...
                      {!baseStats && (
                        ...
                        >
                          Relative gains shown (e.g. +2 Fv). Exact ceilings
                          visible once stats are recorded.
                        </Typography>
                      )}
                    </Box>
                  )
